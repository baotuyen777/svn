...
//        exit();
//    }
//    $redirect_uri = admin_url('admin-ajax.php?action=qsoft_facebook_callback2');
...
function qsoft_facebook_callback_step() {
    if (get_option('qsoft_facebook_enabled') == 1) {
        $app_id = get_option('qsoft_facebook_app_id');
        $app_secret = get_option('qsoft_facebook_app_secret');
    } else {
...
// Get access token
    $getResponse = explode("&", $response);
    $access_token = str_replace('access_token=', '', $getResponse[0]);
...
add_action('wp_ajax_qsoft_facebook_callback', 'qsoft_facebook_callback_step');
add_action('wp_ajax_nopriv_qsoft_facebook_callback', 'qsoft_facebook_callback_step');
